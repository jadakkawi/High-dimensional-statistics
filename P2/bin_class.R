...
summary(GLM1)
ppv=1-pchisq(314.3-57.34, df=404-392)
ppv
...
Classif1 <- glm2$fitted.values >= 0.5
...
mod2 <- glm(train$AH_bin_factor ~ res$scores[,1] + res$scores[,2] + res$scores[,3] +
              res$scores[,4] + res$scores[,5], data = train,family=binomial(logit))
summary(mod2)
...
#And the 1st, 3rd and 4th PCs are kept in the final model.
...
mod3 <- glm(train$AH_bin_factor ~ res$scores[,1] + res$scores[,2] + res$scores[,3] +
              + res$scores[,5], data = train,family=binomial(logit))
summary(mod3)

plot(mod3$linear.predictors, mod3$fitted, col = as.numeric(train[['AH_bin_factor']]))

fitProb3 <- linearPredict3 <- NULL
for(i in 1:dim(test)[1])
{
  ind_i <- c(1, test$PT08.S1.CO.[i], test$NMHC.GT.[i], test$PT08.S2.NMHC.[i],
             test$NOx.GT.[i], test$PT08.S3.NOx.[i], test$PT08.S4.NO2.[i], 
